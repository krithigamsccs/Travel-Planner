...
const UpdatePackage = () => {
  ...
  const handleImageSubmit = () => {
    if (
      images.length > 0 &&
      images.length + formData.packageImages.length < 11
    ) {
      setUploading(true);
      setImageUploadError(false);
      ...
      setImageUploadError("You can only upload up to 10 images per package.");
    }
  };
      ...
      >
        ...
        <div className="w-full sm:w-[30%] shadow-md rounded-xl p-3 h-max flex flex-col gap-2">
          <label htmlFor="packageImages">
            Images:
            <span className="text-red-700 text-sm">
              (Images must be smaller than 2MB and max 10 images.)
            </span>
          </label>
          <input
            type="file"
